# Description:
#   Office365の日報フォルダにデータを投稿します。
# 
# Dependencies:
#   None
#
# Configuration:
#   NIPPO_FOLDER_NAME 日報を保存するOffice365上の共有フォルダ
#   NIPPO_TEMPLATE_NAME 日報フォルダ内のテンプレートファイル
#   O365_CLIENT_ID 日報ボット用のOffice365アプリClientId
# 
# Commands:
...
searchNippoFile = (name, cb) ->  # any
  office.onedrive.getClient (client) ->
    client.getFileByName "#{NIPPO_FOLDER_NAME}/#{name}", (err, file) ->
      cb file
...
getTodayNippo = (msg, cb) ->
  ...
  download = (file, templ) ->
    office.onedrive.getClient (client) ->
      client.downloadFile file.id, nippoPath, ->
        if not templ
...
uploadTodayNippo = (msg, field, cb) ->
  ...
  fs.readFile nippoPath + ".id", "utf8", (err, fileId) ->
    findNippoFolder msg, (client, folderId) ->
      if not fileId?
        client.createFile nippoName(msg), folderId, (err, file) ->
          if err? then cb null
          else
